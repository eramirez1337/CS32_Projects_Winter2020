...
bool pathExists(string maze[], int nRows, int nCols, int sr, int sc, int er, int ec)
{
    ...
    coordStack.push(Coord(sr,sc)); //Push starting coord (sr,sc) onto stack
    maze[sr][sc] = 'S'; //Indicates (S)tarting position
    while (coordStack.empty()==false) //While the stack is not empty
    {
        Coord currentPoint = coordStack.top();
        coordStack.pop();
        ...
        else
        {
            ...
            {
                coordStack.push(Coord(currentPoint.r(), currentPoint.c()+1)); //Push (r,c+1) onto queue
            ...
            {
                coordStack.push(Coord(currentPoint.r()+1, currentPoint.c())); //Push (r+1,c) onto queue
