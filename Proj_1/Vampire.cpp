...
void Vampire::move()
{
    ...
    if (attemptMove(*m_arena, randInt(0, NUMDIRS-1), m_row, m_col))
    {
        ...
        {
            if (m_health == INITIAL_VAMPIRE_HEALTH) { //Here is where we can begin to record the Vamp's health when it becomes poisoned
                m_arena->history().record(m_row, m_col); //Since it was able to land on this spot, record outputs true and records a move
            }
