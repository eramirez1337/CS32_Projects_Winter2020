...
void Game::play()
{
    ...
    while ( ! player->isDead()  &&  m_arena->vampireCount() > 0)
    {
        ...
        if (msg == "Press enter to continue.") { //New snippet of code that allows the Game class to recognize the h input and perform the specified actions
            m_arena->history().display();
            cout << msg;
            cin.ignore(10000, '\n');
            m_arena->display("");
            continue; //Prevents the move counter to increase as h inputs aren't moves
        }
...
//Since recommendMove and computeDanger are only used by the Game class, it is necessary to place them here instead of utilities
